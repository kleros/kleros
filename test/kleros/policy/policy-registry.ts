...
describe("PolicyRegistry", () =>
  it("Should allow setting subcourt policies.", async () => {
    ...
    const policyRegistry = (await PolicyRegistry.deploy(governor.address)) as PolicyRegistry;

    const policies = ["https://a.b.com", "https://c.d.com", "https://e.f.com", "https://g.h.com"];

    // Verify policy update events were emitted.
    for (const policy of policies)
      await expect(policyRegistry.connect(governor).setPolicy(subcourtID, policy))
