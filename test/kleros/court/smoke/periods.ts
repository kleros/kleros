...
describe("Smoke: Dispute - Peroids", () => {
  ...
  describe("Revert Execution", () => {
    ...
    it("Should fail to pass Evidence Period: not enough juror drawn", async () => {
      ...
      await expect(klerosLiquid.passPeriod(disputeID)).to.be.revertedWith("The dispute has not finished drawing yet.");
    });
    ...
    it("Should fail to pass Appeal and Execution periods", async () => {
      const { klerosLiquid, subcourt, jurors } = await useStakedSetup(numberOfJurors);
      ...
      for (const juror of Object.values(jurors)) {
        ...
        if (voteIDs.has(juror.address))
          await klerosLiquid.connect(juror).castVote(disputeID, [voteId], NUMBER_OF_CHOICES, 0);
      }

      await klerosLiquid.passPeriod(disputeID);
      await expect(klerosLiquid.passPeriod(disputeID)).to.be.revertedWith("The appeal period time has not passed yet.");

      await increaseTime(subcourt.timesPerPeriod[Period.appeal]);
      await klerosLiquid.passPeriod(disputeID);

      await expect(klerosLiquid.passPeriod(disputeID)).to.be.revertedWith("The dispute is already in the last period.");
    });
  });
});
