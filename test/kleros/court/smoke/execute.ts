...
describe('Smoke: Dispute - Execution', () => {
  it('Should execute penalty/reward distribution', async () => {
    const { klerosLiquid, choices, pnk, jurors } = await useVotedSetup(
      numberOfJurors
    );
    ...
    const jurorsBalancesBefore = await getJurorsBalances(
      Array.from(choices.keys()),
      ethers.provider
    );

    const jurorsPnkBalancesBefore = await getJurorsBalances(
      Array.from(choices.keys()),
      pnk
    );

    const tx = await klerosLiquid
      .connect(jurors[0])
      .execute(disputeID, 0, 2 * numberOfJurors, {
        gasPrice,
      });
    expect(await klerosLiquid.lockInsolventTransfers()).to.be.equal(true);

    const jurorsBalancesAfter = await getJurorsBalances(
      Array.from(choices.keys()),
      ethers.provider
    );

    const jurorsPnkBalancesAfter = await getJurorsBalances(
      Array.from(choices.keys()),
      pnk
    );
    ...
    for (const [juror, _] of jurorsBalancesBefore) {
      ...
      voteCount.winningChoice.eq(jurorChoice) &&
        expect(jurorsBalancesAfter.get(juror)).to.equal(
          jurorsBalancesBefore.get(juror)?.add(ETHReward)
        );

      expect((await klerosLiquid.jurors(juror)).lockedTokens).to.be.equal(0);
      expect(jurorsPnkBalancesAfter.get(juror)).to.be.equal(
        jurorsPnkBalancesBefore.get(juror)
      );
    }
  });
});
