...
describe("Smoke: Governor - List Submission", () => {
  ...
  it("Should set correct values in a newly submitted list and validate the emitted event", async () => {
    const { appeableArbitrator, governor, users, args } = await useInitialSetup();
    ...
    const txHash1 = parseInt(soliditySha3(governor.address, 10, encodedData[0]) as string, 16);
    const txHash2 = parseInt(
      soliditySha3(appeableArbitrator.address, BigNumber.from(1).pow(17).toNumber(), encodedData[1]) as string,
      16
    );
    ...
    expect(submission.deposit, "The deposit of the list is incorrect").to.equal(args.submissionDeposit());

    const submissionLength = await governor.getNumberOfTransactions(0);
    expect(submissionLength, "The number of transactions is incorrect").to.equal(2);
    ...
    for (let i = 0; i < indices.length; i++) {
      ...
      txs.push(tx);
      expect(tx.target, `The target of the ${i}. transaction is incorrect`).to.equal(addresses[i]);

      expect(tx.value, `The value of the ${i}. transaction is incorrect`).to.equal(values[i]);

      expect(tx.data, `The data of the ${i}. transaction is incorrect`).to.equal(encodedData[i]);
    }
    ...
    if (txHash1 < txHash2) {
      hash1 = soliditySha3(soliditySha3(txs[0].target, txs[0].value.toNumber(), txs[0].data) as string, 0);
      hash2 = soliditySha3(txs[1].target, txs[1].value.toNumber(), txs[1].data);
    } else {
      hash1 = soliditySha3(soliditySha3(txs[1].target, txs[1].value.toNumber(), txs[1].data) as string, 0);
    ...
    await expect(
      governor.connect(users.submitter2).submitList([governor.address], [100], "0xfdea", [2], listDescription, {
  ...
  it("should correctly withdraw submitted list", async () => {
    ...
    expect(submittedLists.length, "The submission count is incorrect").to.equal(2);

    let sessionInfo = await governor.sessions(0);
    expect(sessionInfo.sumDeposit, "The sum of submission deposit is incorrect").to.equal(
      args.submissionDeposit().mul(2)
    );
    ...
    expect(submittedLists.length, "The submission count is incorrect").to.equal(1);

    sessionInfo = await governor.sessions(0);
    expect(sessionInfo.sumDeposit, "The sum of submission deposit is incorrect").to.equal(args.submissionDeposit());
  });
  ...
  describe("Revert Execution", () => {
    it("Should fail when submission list args are not of the same length", async () => {
      ...
      const submissionDeposit = args.submissionBaseDeposit.add(args.arbitrationFee);
    ...
    it("Should fail after withdraw timeout", async () => {
      ...
      await expect(governor.connect(users.submitter1).withdrawTransactionList(0, listInfo.listHash)).to.be.reverted;
    });
    ...
    it("Should fail on unauthorized withdrawl", async () => {
      ...
      await expect(governor.connect(users.submitter2).withdrawTransactionList(0, listInfo.listHash)).to.be.reverted;
    });
