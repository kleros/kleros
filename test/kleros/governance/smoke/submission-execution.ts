...
describe("Smoke: Governor - Submission Execution", () => {
  ...
  it("Should advance to approval period if no submitted list", async () => {
    ...
    await expect(governor.connect(users.deployer).executeSubmissions()).to.be.reverted;
  ...
  it("Should approve a list if one submitted list", async () => {
    ...
    expect(submittedLists.length, "The submission count should be set to 0 right after approval").to.equal(0);
  });

  it("Should create a dispute if more than one submitted list", async () => {
    const { governor, appeableArbitrator, args, users } = await useListSubmissionSetup();

    await governor.submitList([appeableArbitrator.address], [10], "0x2462", [2], listDescription, {
  ...
  it("Should check that submissions are working in the new submitting session", async () => {
    ...
    await governor.connect(users.submitter2).submitList([], [], "0x00", [], listDescription, {
      value: args.submissionDeposit(),
    });

    const submittedLists = await governor.getSubmittedLists(1);
    expect(submittedLists.length, "The submission count in the new session is incorrect").to.equal(1);

    const numberOfLists = await governor.getNumberOfCreatedLists();
    expect(numberOfLists, "The number of created lists is incorrect").to.equal(2);

    const latestSession = await governor.getCurrentSessionNumber();
    let sessionInfo = await governor.sessions(latestSession);
    expect(sessionInfo.status, "Status should be NoDispute in the new session").to.equal(Status.NoDispute);

    sessionInfo = await governor.sessions(latestSession.sub(1));
    expect(sessionInfo.status, "Previous session should have status resolved").to.equal(Status.Resolved);

    expect(sessionInfo.sumDeposit, "The sum of the deposits should be 0 in the previous session").to.equal(0);
  });
});
