...
describe('SortitionSumTreeFactory', () => {
  it('Should successfully keep track of ID ownership of values and draw them from the tree appropriately.', async () => {
    await deployments.fixture(
      ['SortitionSumTreeFactory', 'ExposedSortitionSumTreeFactory'],
    ...
    expect(await sortitionSumTreeFactory._draw(tree.key, 13)).to.be.equal(
      candidates.ali.ID
    );
    expect(await sortitionSumTreeFactory._draw(tree.key, 27)).to.be.equal(
      candidates.deli.ID
    );
    ...
    // Test stake view
    for (const candidate of Object.values(candidates)) {
      const stakeOfCandiate = await sortitionSumTreeFactory._stakeOf(
        tree.key,
        candidate.ID
      );
