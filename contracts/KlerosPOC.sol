...
contract KlerosPOC is Arbitrator {
    ...
    function oneShotTokenRepartition(uint _disputeID) public onlyDuring(Period.Execution) {
        ...
        for (uint i=0;i<=dispute.appeals;++i) {
            ...
            if (winningChoice!=0 || (dispute.voteCounter[dispute.appeals].voteCount[0] == dispute.voteCounter[dispute.appeals].winningCount)) {
                dispute.appealsRepartitioned[i].totalToRedistribute=0;
    ...
    */
    ...
    /** @dev Return the repartitioned token amount for a juror in a specific appeal.
     *  @param _disputeID ID of the dispute.
     *  @param _appeal The appeal of the dispute.
     *  @param _ruling The ruling of the juror.
     *  @return repartitionedPNK The amount of PNK lost or won through the repartition.
     */
    function repartitionedPNK(uint _disputeID, uint _appeal, uint _ruling) public constant returns(uint repartitionedPNK) {
        Dispute storage dispute = disputes[_disputeID];
        return _ruling == currentRuling(_disputeID) ?
            // Total / Coherent Jurors
            dispute.appealsRepartitioned[_appeal].totalToRedistribute / dispute.appealsRepartitioned[_appeal].nbcoherent
        :
            // Total / (All Jurors - Coherent Jurors)
            - (dispute.appealsRepartitioned[_appeal].totalToRedistribute / (amountJurors(_disputeID) - dispute.appealsRepartitioned[_appeal].nbcoherent));
    }
